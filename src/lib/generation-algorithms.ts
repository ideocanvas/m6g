...
import { generateClassicCombinations, generateFollowOnCombinations } from './algorithms';
 ...
 * Now uses the extracted classic algorithm following IoC pattern
 */
export async function generateV1Combinations(
  ...
  try {
    ...
    // Use the extracted classic algorithm (pure function)
    ...
    console.log("V1 Generation - Classic algorithm results:", {
      count: classicResults.length,
      firstCombination: classicResults[0]?.combination
    });

    return classicResults.map(result => result.combination);
  } catch (error) {
 ...
 * Aligned with reference implementation from MarkSizAPI.js
 * Now uses the extracted follow-on algorithm following IoC pattern
 */
export async function generateV2Combinations(
  ...
  try {
    ...
    // Get historical data for follow-on analysis
    const cutoffDate = new Date();
    cutoffDate.setDate(cutoffDate.getDate() - daysOfHistory);
    ...
    // Use the extracted follow-on algorithm (pure function)
    ...
    if (DEBUG) {
      console.log("\n--- Generation Complete ---");
      console.log(`Total combinations generated: ${followOnResults.length}`);
      console.log("---------------------------\n");
    }

    return followOnResults.map(result => result.combination);
  } catch (error) {
